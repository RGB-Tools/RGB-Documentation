...
5. Before broadcasting the transaction prepared in this way, she passes to Bob a package of data called a [Consignment](state-transitions.md) which contain the organized stash of client-side data already in possession of Alice in addition to the new state. Bob, at this point through the tools coded in [RGB-core library](https://github.com/RGB-WG), and [client-side validation library](state-transitions.md):
   ...
   * through the [Anchors](commitment-layer/csv-w-btc.md#anchors), **verifies the chronological ordering of the** [**witness transactions**](state-transitions.md) and the relate commitments to RGB data contained herein.
...
* **method** is a 1-byte field which indicate the seal closing method, which will be used in the related [witness transaction](terminology/glossary.md#--). It's either [tapret](commitment-layer/csv-w-btc.md#tapret) or [opret](commitment-layer/csv-w-btc.md#tapret).
